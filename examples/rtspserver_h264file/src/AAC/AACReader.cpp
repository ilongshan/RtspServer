...
AACFrame *AllocAACFrame(int buffersize)
{
    ...
    frame = (AACFrame*)malloc(sizeof(AACFrame));
    frame->buffer = (unsigned char*)malloc(buffersize);
...
AACFrame* AACReader::getNextFrame()
{
    ...
    AACFrame *aacFrame = AllocAACFrame(buffersize);

    aacFrame->adtsHeader = adtsHeader;
    aacFrame->bufferSize = buffersize;
    memcpy(aacFrame->buffer, buffer, buffersize);
